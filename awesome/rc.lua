...
nconf.presets.critical.bg = beautiful.bg_urgent
nconf.presets.critical.fg = beautiful.fg_urgent
...
mypowermenu = awful.menu({ items = powermenu
                        })

mylauncher = awful.widget.launcher({ image = beautiful.awesome_icon,
                                     menu = mymainmenu })

mypowerlauncher = awful.widget.launcher({ image = beautiful.awesome_icon,
                                     menu = mypowermenu })
...
awful.spawn.easy_async("bash -c \"acpi | grep -oP '\\d+%'\"", function(stdout, stderr, reason, exit_code)
    batterytext.text = "BAT:"..stdout
end)
brighttext = wibox.widget.textbox()
awful.spawn.easy_async("xbacklight -get", function(stdout, stderr, reason, exit_code)
    brighttext.text = "BRI:"..stdout
end)
voltext = wibox.widget.textbox()
awful.spawn.easy_async("bash -c \"pactl get-sink-volume @DEFAULT_SINK@ 150% | grep -oP \'\\d+%\'\"", function(stdout, stderr, reason, exit_code)
    voltext.text = "VOL:"..stdout
end)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mywibox:setup {
        ...
        nil,--wibox.widget.systray(), -- Middle widget
	{ -- Right widgets
            layout = wibox.layout.fixed.horizontal,
            --mykeyboardlayout,
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey,  "Shift"  }, "e", function () mypowermenu:show() end,
    ...
    awful.key({ modkey,           }, "c", function () awful.spawn("killall picom") end,
              {description = "kill picom", group = "launcher"}),
    awful.key({ modkey,     "Shift"      }, "c", function () awful.spawn("picom -b") end,
              {description = "start picom", group = "launcher"}),
